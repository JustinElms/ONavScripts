...
csv_name="${user_id}_profiling_results.csv"
# add -l to suppress logfile and -c to suppress csv output

python api_profiling_driver.py --url $url --config $config --csv $csv_name --prof $prof_path --id $user_id -a $max_attempts -t $max_time
